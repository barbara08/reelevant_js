...
​
let value = buscarConCondicion (costes[0], (x,y) => (x>y));
console.log (`El mes ${meses[value.posicion]} fue el de mayor coste con un valor de ${value.valor}`);
let suma = 0;
for (let i=0; i < MAX_MESES; i++) {
    suma += costes[1][i];
}
...
console.log (`El mes ${meses[value.posicion]} fue el de mayor coste con un valor de ${value.valor}`);
value = buscarConCondicion (costes[1], (x,y) => (x<y));
console.log (`El mes ${meses[value.posicion]} fue el de menor coste con un valor de ${value.valor}`);
value = buscarConCondicion (costes, (x,y) => (x[11] < y[11]));
console.log (`El departamento ${departamentos[value.posicion]} fue el de menor coste en diciembre con un valor de ${value.valor[11]}`);
